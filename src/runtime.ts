...
import { PublishParams, SuiCoin } from "./common.js";
import fs from "fs";
...
export class Runtime {
  ...
  getClusterUrl() {
    return getFullnodeUrl(this.network as any);
  }
  ...
  publishPackageTransaction(params: PublishParams) {
  ...
  async getUpgradeCap(packageId: string) {
  ...
  async getTreasuryCap(packageId: string) {
  ...
  async waitAndReturn(digest: string) {
  ...
  getTransactionLink(digest: string) {
  ...
  async getCoinMetadata(coinTypeOrAlias: string): Promise<CoinMetadata | null> {
