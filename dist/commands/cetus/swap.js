...
import { Percentage, adjustForSlippage, } from "@cetusprotocol/cetus-sui-clmm-sdk";
...
const swapCommand = {
    ...
    builder: (yargs) => {
        return yargs
            ...
            .option("amount", {
            type: "string",
            description: "amount of coin to swap. if a2b is true, this is amount of coin a to swap, or amount of coin b to swap.this is human readable number",
            demandOption: true,
            default: "0.1",
        })
            .option("a2b", {
    ...
    async handler(args) {
        ...
        if (swapTxn) {
            console.log(`swapTxn: ${swapTxn?.digest} , ${runtime.getTransactionLink(swapTxn?.digest)}`);
        }
    },
};
