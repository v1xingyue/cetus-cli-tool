...
const mint = {
    ...
    builder: (yargs) => {
        return yargs
            ...
            alias: "r",
            type: "string",
            description: "recipient address, default is current loadRuntime address",
            default: "",
        })
            .option("amount", {
            alias: "a",
    ...
    async handler(args) {
        try {
            ...
            if (!meta) {
                ...
                const realAmount = BigInt(args.amount) * BigInt(10) ** BigInt(meta.decimals);
                console.log(`realAmount ${realAmount}`);
                const tx = runtime.getMintTransaction(treasuryCap.data?.objectId, realAmount.toString(), args.recipient, args.freeze, coinType);
                const resp = await runtime.signAndExecute(tx);
                console.log(`mint transaction : ${runtime.getTransactionLink(resp.digest)}`);
            }
        }
        catch (error) {
