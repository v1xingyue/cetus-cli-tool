...
const createPool: CommandModule<Option, Option> = {
  ...
  builder: (yargs) => {
    return yargs
      ...
      .option("init-price", {
        type: "number",
        description:
          "init price of coin a , such as 1a = 0.1b , then the price will be 0.1",
        demandOption: true,
        default: 1,
      })
      .option("amount-a", {
  ...
  async handler(args: Option) {
    ...
    const fix_coin_amount = new BN(
      Math.floor(parseFloat(args.amountA) * Math.pow(10, metaA.decimals))
    );
    ...
    console.log(
      `create pool transaction : ${chalk.green(
        runtime.getTransactionLink(resp.digest)
      )}`
    );
  },
};
