...
const removeLiquidity: CommandModule<Option, Option> = {
  command: "remove-liquidity",
  describe:
    "remove liquidity from a position (based on Cetus remove_liquidity documentation)",
  ...
  async handler(args: Option) {
    ...
    try {
      ...
      const removeLiquidityTransactionPayload =
      ...
      console.log(
        `remove liquidity transaction link: ${runtime.getTransactionLink(
          result.digest
        )}`
      );
    } catch (error) {
